module murosync_gt_wrapper #(
  ...
  // Hold helper blocks in reset until the GT says PMA reset done (как ты делал)
  wire gtwiz_userclk_tx_reset = ~(&txpmaresetdone_out);
  wire gtwiz_userclk_rx_reset = ~(&rxpmaresetdone_out);

  gtwizard_ultrascale_0_example_gtwiz_userclk_tx u_userclk_tx (
  ...
  gtwizard_ultrascale_0_example_gtwiz_userclk_rx u_userclk_rx (
